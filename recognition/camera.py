	...
	def take_image(self, Id, name):
		...
		while (True):
			...
			for (x, y, w, h) in faces:
				...
				cv2.imwrite(settings.MEDIA_ROOT + "/" + "TrainingImage" + "/" + name + "." + str(Id) + '.' + str(sampleNum) + ".jpg", gray[y:y + h, x:x + w])
				# display the frame
		...
		row = [Id, name]
		with open(settings.MEDIA_ROOT + '/' + 'StudentDetails.csv', 'a+') as csvFile:
	...
	def track_image(self):
		...
		for (x, y, w, h) in faces:
			...
			if (conf > 75):
				...
				cv2.imwrite(os.path.join(os.path.join(settings.MEDIA_ROOT,"ImagesUnknown"), f"{Image}{noOfFile}.jpg"), im[y:y + h, x:x + w])
